...
if [ "$#" -ne 2 ]; then
  ...
  echo "originalFile = Name of the file from which the preprocessed file is projected"
  exit
fi

INPUT=$1              # Name of the input file
ORIGINAL=$2           # Name of the unprocessed file
...
do
  ...
  do  
    for k in `seq 0 3`;
    do
      root -l -b -q 'plotting/checkPreprocess.C("'${INPUT}'",4,'$j','$i','$k',"'${ORIGINAL}'")' # regular jet-track correlations for leading and subleading jets
      root -l -b -q 'plotting/checkPreprocess.C("'${INPUT}'",6,'$j','$i','$k',"'${ORIGINAL}'")' # pT weighted jet-track correlations for leading and subleading jets
    done
