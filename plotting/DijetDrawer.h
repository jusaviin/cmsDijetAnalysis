...
public:
  ...
  void SetDrawDijetHistograms(const bool drawOrNot, const int normalizeXjMatrix = 0, const bool wideBins = false);  // Setter for drawing dijet histograms
  ...
  void SetDrawInclusiveTracksUncorrected(const bool drawOrNot);   // Setter for drawing inclusive tracks
  void SetDrawAllInclusiveTracks(const bool drawInclusive, const bool drawUncorrected);   // Setter for drawing inclusive tracks
...
private:
  ...
  int fNormalizeXjMatrix;      // 0 = Draw counts. 1 = Draw probability for gen xj given reco xj. 2 = Draw probability for reco xj given gen xj.
