...
void longRangeGraphPlotter(){
  ...
  TString graphFileName = "flowGraphs_PbPbMC2018_genJets_subeNon0_fakeJetV2_correctedJetHadron_correctedDihadronWideDeltaEta_2021-04-29.root";
  ...
  TString fileLegend[] = {"Gen jets, fake v_{2}, HiForest EP", "Gen jets, fake v_{2}, manual EP", "Gen jets, nominal", "MC, only gluon jets",  "MC 4%", "MC 15%", "MC 16%", "Flow MC", "MC+5% Q < 2.8"};
  ...
  TString saveComment = "_genToFakeComparison";              // String to be added to saved file names
        ...
        for(int iAsymmetry = firstDrawnAsymmetryBin; iAsymmetry <= lastDrawnAsymmetryBin; iAsymmetry++){
          ...
          if(drawJetHadronVnFileComparison){
            ...
            if(drawRatios){
              ...
              legend = new TLegend(0.22,0.74,0.52,1); // 0.23,0.19,0.53,0.39
              legend->SetFillStyle(0);legend->SetBorderSize(0);legend->SetTextSize(0.05);legend->SetTextFont(62);
              legend->SetHeader(Form("Cent: %.0f-%.0f%%%s", centralityBinBorders[iCentrality], centralityBinBorders[iCentrality+1], asymmetryString[iAsymmetry].Data()));
              ...
              for(int iFile = 0; iFile < maxFileIndex; iFile++){
                ...
                if(iFile == 0){
                  drawer->DrawGraph(ratioGraphJetHadron[iFile][iAsymmetry][iCentrality], 0, maxTrackPt, 0, 2, "Track p_{T} (GeV)", Form("Jet-hadron V_{%d} ratio",iFlow+1), " ", "p");
                } else {
          ...
          if(drawHadronVnFileComparison){
            ...
            if(drawRatios){
              ...
              for(int iFile = 0; iFile < maxFileIndex; iFile++){
                ...
                if(iFile == 0){
                  drawer->DrawGraph(ratioGraphHadron[iFile][iAsymmetry][iCentrality], 0, maxTrackPt, 0.6, 1.4, "Track p_{T} (GeV)", "Hadron v_{2} ratio", " ", "p");
                } else {
