...
void longRangeGraphPlotter(){
  ...
  TString graphFileName = "flowGraphs_PbPbMC2018_genJets_correctedJetHadron_correctedDihadronFromRecoJets_2021-04-23.root";
  ...
  TString fileLegend[] = {"Gen jets, all sube", "Fake v_{2}, subeNon0", "Calo MC", "Flow MC", "MC+5%, Q < 2.2",  "MC+5%, Q < 2.5", "MC+5% Q < 2.8"};
...
//                                   {0.0546, 0.0556, 0.0475, 0.05}};
  ...
  // Testing corrections with Q from integrated bins, only 0-10 actually done. Done with 4 % centrality shift and calo jets
  double manualCorrection[3][4] = {{0.0845, 0.0828, 0.0795, 0.0642},
                                   {0.0802, 0.0834, 0.0919, 0.0979},
                                   {0.0512, 0.0492, 0.0490, 0.0479}};
  
//  // Testing corrections with Q from integrated bins, only 0-10 actually done. Done with 4 % centrality shift and flow jets
//  double manualCorrection[3][4] = {{0.1087, 0.1128, 0.1180, 0.1022},
//                                   {0.0908, 0.0952, 0.0978, 0.0966},
//                                   {0.0340, 0.0351, 0.0358, 0.0286}};
  
//    // Testing corrections with Q from integrated bins, only 0-10 actually done. Done with 4 % centrality shift and pfcs jets
...
//                                   {0.0546, 0.0556, 0.0475, 0.05}};
  ...
  double jetHadronZoomTable[4] = {0.1, 0.035, 0.005, 0.01};
      ...
      for(int iFlow = firstDrawnVn-1; iFlow <= lastDrawnVn-1; iFlow++){
        for(int iAsymmetry = firstDrawnAsymmetryBin; iAsymmetry <= lastDrawnAsymmetryBin; iAsymmetry++){
          ...
          if(drawHadronVnFileComparison){
            ...
            for(int iFile = 0; iFile < nComparisonFiles; iFile++){
              for(int iPoint = 0; iPoint < maxPtBin; iPoint++){
                flowGraphHadron[0][iAsymmetry][iCentrality][iFlow]->GetPoint(iPoint, xPoint1, yPoint1);
                yError1 = flowGraphHadron[0][iAsymmetry][iCentrality][iFlow]->GetErrorY(iPoint);
                flowGraphHadron[iFile+1][iAsymmetry][iCentrality][iFlow]->GetPoint(iPoint, xPoint2, yPoint2);
                yError2 = flowGraphHadron[iFile+1][iAsymmetry][iCentrality][iFlow]->GetErrorY(iPoint);
              ...
              ratioGraphHadron[iFile][iAsymmetry][iCentrality]->SetMarkerStyle(fullMarkers[iFile+1]);
              ratioGraphHadron[iFile][iAsymmetry][iCentrality]->SetMarkerSize(1.3);
              ratioGraphHadron[iFile][iAsymmetry][iCentrality]->SetMarkerColor(fileColors[iFile+1]);
              
              legend->AddEntry(ratioGraphHadron[iFile][iAsymmetry][iCentrality], Form("%s / %s", fileLegend[iFile+1].Data(), fileLegend[0].Data()), "p");
              
              if(iFile == 0){
                drawer->DrawGraph(ratioGraphHadron[iFile][iAsymmetry][iCentrality], 0, maxTrackPt, 0.85, 1.15, "Track p_{T} (GeV)", "Ratio", " ", "p");
              } else {
            ...
            for(int iFile = 0; iFile < nComparisonFiles+1; iFile++){
              ...
              if(fitJetVn){
                if(iFile > 0){
                  ...
                  flowGraphJet[iFile][iAsymmetry][iCentrality][iFlow]->Fit("pol0","0","",0,2); // TODO: DEBUG DEBUG REMOVE FIT RANGE
                }
