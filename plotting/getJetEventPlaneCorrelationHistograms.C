void getJetEventPlaneCorrelationHistograms(){
  ...
  // Open the data file
  TFile *inputFile = TFile::Open("data/PbPbMC2018_GenGen_akCaloJet_onlyRegular_eventPlanes_fakeJetV3_sN0_multWeight_jetEta1v3_2022-02-03.root");
  ...
  TFile *outputFile = new TFile("eventPlaneCorrelation/jetEventPlaneDeltaPhi_PbPbMC2018_genJets_fakeJetV3_multiplicityWeight_allEventPlanes_jetEta1v3_2022-02-03.root","UPDATE");
  for(int iOrder = 0; iOrder < nEventPlaneOrder; iOrder++){
    for(int iCentrality = 0; iCentrality < nCentralityBins; iCentrality++){
