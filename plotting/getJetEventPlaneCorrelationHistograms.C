void getJetEventPlaneCorrelationHistograms(){
  ...
  // Open the data file
  TFile *inputFile = TFile::Open("data/PbPbMC2018_GenGen_akCaloJet_onlyRegular_noCentShift_subeNon0_fakeJetVn_jetEta1v6_noMix_2022-02-08.root");
  ...
  TFile *outputFile = new TFile("eventPlaneCorrelation/jetEventPlaneDeltaPhi_PbPbMC2018_genJets_fakeJetVn_noCentShift_updatedEventPlanes_jetEta1v3_2022-02-08.root","UPDATE");
  for(int iOrder = 0; iOrder < nEventPlaneOrder; iOrder++){
    for(int iCentrality = 0; iCentrality < nCentralityBins; iCentrality++){
