...
do
  ...
  do  
    for k in `seq 0 3`;
    do
      root -l -b -q 'plotting/combineCorrelations.C("'${FIRSTFILE}'","'${SECONDFILE}'","'${OUTPUT}'",4,'$FIRSTWEIGHT','$SECONDWEIGHT','$j','$i','$k')' # Regular jet-track correlations for leading and subleading jets
#      root -l -b -q 'plotting/combineCorrelations.C("'${FIRSTFILE}'","'${SECONDFILE}'","'${OUTPUT}'",5,'$FIRSTWEIGHT','$SECONDWEIGHT','$j','$i','$k')' # Uncorrected jet-track correlations for leading and subleading jets
      root -l -b -q 'plotting/combineCorrelations.C("'${FIRSTFILE}'","'${SECONDFILE}'","'${OUTPUT}'",6,'$FIRSTWEIGHT','$SECONDWEIGHT','$j','$i','$k')' # pT weighted jet-track correlations for leading and subleading jets
    done
